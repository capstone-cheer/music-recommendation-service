...
from recommendation.spotify import SpotifyManager
import pandas as pd


class Recommend:
    def __init__(self, external_path):
        self.seed_genre_list = ["rock_metal", "dance", "pop",
                                "rap_hiphop", "ballade", "electronica", "idol", "folk", "indie"]
        ...
        self.s2v_model = KeyedVectors.load_word2vec_format(
            os.path.join(self.MODEL_PATH, self.S2V_NAME))
    ...
    def get_by_playlist(self, seed, category, genre):
        ...
        for song_id in seed:
            try:
                ...
                topn_songs = self.s2v_model.similar_by_word(
                    str(song_id), topn=10)
    ...
    def get_gnr_score(self, genre):
        ...
        try:
            tag_songs_to_series = pd.Series(
                self.gnr_scoring_model['songs'][genre])
        except KeyError:
            tag_songs_to_series = pd.Series(
                self.gnr_scoring_model['songs'][random.choice(self.seed_genre_list)])
